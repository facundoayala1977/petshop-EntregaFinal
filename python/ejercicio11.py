#Funciones definidas por el usuario

from colorama import Fore, Back, init
init()

import datetime

#Funcion Agregar Productos

productos = []

def agregar_productos():
    print("\nProducto a agregar\n")
    producto = input ("Ingrese el nombre del producto: \n").strip().capitalize()
    if producto == "" or producto.isdigit():
        print ("Debe ingresar una cadena de caracteres!!!\n")
        return False #Si la entrada no es válida, finaliza la función
    
    precio = input ("Ingrese el precio del producto: $ \n") .strip()
        
    if not precio.isdigit() or int(precio) <= 0:
        print ("El precio ingresado debe ser un número entero mayor a 0, vuelva a intentarlo.\n")
        return False #Si la entrada no es válida, finaliza la función
    
    precioAgregado = int(precio)
    productos.append([producto, precioAgregado])
    fecha_hora = datetime.datetime.now()
    print (Fore.MAGENTA + f"\n Producto '{producto}' agregado exitosamente el: ", fecha_hora.strftime("%d de %B de %Y"), " a las ",fecha_hora.strftime("%H:%M:%S"))
    return True #Confirma la operación de agregar producto

#Funcion Consultar Productos

def consultar_productos():
    if productos:
        print(Fore.LIGHTMAGENTA_EX + "\nListado de Productos:")
        for i, producto in enumerate (productos, start=1):
            print(f"\n{i}. {producto}\n")
        return productos
        
    else: 
        print(Fore.RED + "\nLa lista de productos se encuentra vacía en estos momentos.\n")
        return None
            
#Función Eliminar Productos

def eliminar_productos():
    global productos
    if productos:
        producto = input("Ingresá el nombre del producto que querés eliminar: ").strip().capitalize()
        
        # Buscar el producto por nombre dentro de la lista de listas
        for item in productos:
            if item[0] == producto:
                productos.remove(item)
                fecha_hora = datetime.datetime.now()
                print(f"Producto '{producto}' eliminado con éxito el ",fecha_hora.strftime("%d de %B de %Y"), " a las ",fecha_hora.strftime("%H:%M:%S"))
                return  # Salimos de la función una vez eliminado
        
        print(f"El producto '{producto}' no se encuentra en el listado.\n")
    else:
        print(Fore.RED + "\nLa lista de productos se encuentra vacía. No hay nada para borrar...")
        
#Función Mostrar Menú

def mostrar_menu():
    while True:
        print(Fore.YELLOW +"===================================================")
        print (Fore.YELLOW +"**** Ingreso de Productos, Categorías y Precios****")
        print(Fore.YELLOW +"===================================================\n")
        print (Fore.GREEN + "-1- Agregar un producto ")
        print (Fore.GREEN + "-2- Consultar productos")
        print (Fore.GREEN + "-3- Eliminar un producto")
        print (Fore.GREEN + "-4- Salir ")
    
        opcion = input(Fore.BLUE + "\nElija una opción del menú: \n")
        #validación
        if opcion == "1":
            agregar_productos()
        elif opcion == "2":
            consultar_productos()
        elif opcion == "3":
            eliminar_productos()
        elif opcion == "4":
            print(Fore.RED +"Gracias por usar el servicio!!! Hasta Luego...\n")
            break
        else:
            print (Fore.RED +"\nOpción Inválida!!! Debe ingresar una opción del 1 al 4. Intente nuevamente...")
            
mostrar_menu()